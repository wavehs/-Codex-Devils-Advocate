...
<p align="center">
  <img src="assets/C048235B-D8A8-42E4-A298-E52B5E3388A0.png" alt="Codex Devil's Advocate — токен-эффективное adversarial review для Codex" width="100%" />
</p>
...
<p align="center">
  <strong>Токен-эффективный adversarial review skill для Codex.</strong><br/>
  Он не пытается подтвердить, что код хороший — он пытается <em>сломать</em> его.
</p>
...
## Перестаньте спрашивать coding agent, правильно ли он написал код.

Попросите другого агента доказать, что код **неправильный**.

У типичного AI code review есть две проблемы: либо проверка слишком поверхностная и пропускает реальные баги, либо она сжигает слишком много контекста и токенов из-за нескольких reviewer-агентов и повторного чтения всего репозитория после каждого исправления.

**Codex Devil's Advocate** использует другой подход.

> Один сильный adversarial reviewer. Один полный проход. До двух точечных повторных проверок. Цикл продолжается только из-за подтверждённых дефектов.

Главная цель — **максимум реально найденных ошибок на потраченный токен**.
